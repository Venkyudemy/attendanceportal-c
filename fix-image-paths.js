...
console.log('🎯 CAMERA TO ADMIN IMAGE ROUTING FIX');
console.log('=====================================');
...
mongoose.connect(MONGODB_URI)
  .then(async () => {
    ...
    const uploadsDir = path.join(__dirname, 'Backend', 'uploads', 'employees');
    const diskFiles = {};
    ...
    for (const emp of finalEmployees) {
      if (emp.attendance?.today) {
        ...
        if (today.checkInImage) {
          const exists = fs.existsSync(path.join(__dirname, 'Backend', today.checkInImage));
          console.log('  📸 Check-in:', today.checkInImage, exists ? '✅' : '❌');
        }
        if (today.checkOutImage) {
          const exists = fs.existsSync(path.join(__dirname, 'Backend', today.checkOutImage));
