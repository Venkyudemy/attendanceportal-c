...
router.get('/attendance', async (req, res) => {
  try {
    ...
    const attendanceData = employees.map(emp => ({
      ...
      domain: emp.domain,
      joinDate: emp.joinDate,
      attendance: emp.attendance  // Return FULL attendance object including images
    }));
...
// POST /api/employee/:id/check-in-with-image - Employee check-in with camera image
router.post('/:id/check-in-with-image', upload.single('image'), async (req, res) => {
  try {
    console.log('Check-in with image request for employee ID:', req.params.id);
    ...
    if (existingRecordIndex >= 0) {
      ...
      employee.attendance.records[existingRecordIndex].isLate = isLate;
      employee.attendance.records[existingRecordIndex].timestamp = now.toISOString();
      employee.attendance.records[existingRecordIndex].checkInImage = imagePath;
    } else {
      employee.attendance.records.push({
        date: today,
        checkIn: checkInTime,
    ...
    await employee.save();
    console.log('✅ Check-in with image saved successfully');
    ...
    console.error('Error during check-in with image:', error);
    res.status(500).json({ 
      error: 'Internal server error',
      message: 'Failed to process check-in with image' 
    });
  }
});

// POST /api/employee/:id/check-out-with-image - Employee check-out with camera image
router.post('/:id/check-out-with-image', upload.single('image'), async (req, res) => {
  try {
    console.log('Check-out with image request for employee ID:', req.params.id);
    ...
    employee.attendance.today.hours = hoursWorked;
    employee.attendance.today.timestamp = now.toISOString();
    employee.attendance.today.checkOutImage = imagePath;
    ...
    await employee.save();
    console.log('✅ Check-out with image saved successfully');
    ...
    console.error('Error during check-out with image:', error);
    res.status(500).json({ 
      error: 'Internal server error',
      message: 'Failed to process check-out with image' 
    });
  }
});
