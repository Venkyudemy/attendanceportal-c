...
const EmployeePortal = ({ currentUser }) => {
  ...
  const [cameraType, setCameraType] = useState(null); // 'checkin' or 'checkout'
  ...
  const handleCheckIn = () => {
    ...
    // Show camera for check-in
    setCameraType('checkin');
    setShowCamera(true);
  };

  // Process check-in with captured image
  const processCheckInWithImage = async (imageBlob) => {
    setIsLoading(true);
    setCheckInStatus('Processing check-in...');
    setShowCamera(false);

    try {
      console.log('Attempting check-in with image for user:', currentUser);
      ...
      console.log('📤 Sending image to backend:', {
        endpoint: `${API_URL}/employee/${employeeId}/check-in-with-image`,
        hasImage: !!imageBlob,
        imageSize: imageBlob.size
      });

      const response = await fetch(`${API_URL}/employee/${employeeId}/check-in-with-image`, {
        method: 'POST',
        headers: {
  ...
  const handleCheckOut = () => {
    ...
    // Show camera for check-out
    setCameraType('checkout');
    setShowCamera(true);
  };

  // Process check-out with captured image
  const processCheckOutWithImage = async (imageBlob) => {
    setIsLoading(true);
    setCheckInStatus('Processing check-out...');
    setShowCamera(false);

    try {
      console.log('Attempting check-out with image for user:', currentUser);
      ...
      console.log('📤 Sending check-out image to backend:', {
        endpoint: `${API_URL}/employee/${employeeId}/check-out-with-image`,
        hasImage: !!imageBlob,
        imageSize: imageBlob.size
      });

      const response = await fetch(`${API_URL}/employee/${employeeId}/check-out-with-image`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      console.log('📥 Check-out backend response status:', response.status);
        ...
        <CameraCapture
          ...
          onCapture={(imageBlob) => {
            if (cameraType === 'checkin') {
              processCheckInWithImage(imageBlob);
            } else if (cameraType === 'checkout') {
              processCheckOutWithImage(imageBlob);
            }
          }}
